...
class GetStationListThread extends Thread {	//기상청 연결을 위한 스레드
	...
	String addr="addr=";
	String infoCnt="numOfRows=50"; // 열의 갯수 50
	String xGrid="tmX=",yGrid="tmY="; // 다음 지도에서 변환 받은 위치 정보로 받은 좌표값.
    /*
	public GetStationListThread(boolean receiver, String sido, int fraga){
			...
			public void run() {
				...
				if(tResponse) {        //문서를 다 읽었다
                    ...
                    if (getAPI == 1) {
                        ...
                        ListActivity.NearStationThreadResponse(sStationName, sAddr, sTm);
                    }
                    */
                        WeatherFragment.NearStationThreadResponse(sStationName, sAddr, sTm);
