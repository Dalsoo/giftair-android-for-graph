...
class GetFindDustThread extends Thread {	//기상청 연결을 위한 스레드
	...
	String Servicekey="ServiceKey="; // 서비스 발급 받은 키
	...
	String searchDate="dataTerm=daily"; // 기간은 매일
	String station="stationName="; // 지역 이름
	String infoCnt="numOfRows=50"; // 열의 갯수는 50
	...
	public GetFindDustThread(boolean receiver, String dong){
		...
		isreceiver=receiver;
        try {
            dongName = URLEncoder.encode(dong, "UTF-8"); // 지역이름을 UTF-8을 써서 변환
        }catch (UnsupportedEncodingException e) {

        }

		bTotalCount=bDate=bSo2Value=bCoValue=bO3Value=bNo2Value=bPm10Value=bKhaiValue=bKhaiGrade=bSo2Grade=bCoGrade=bO3Grade=bNo2Grade=bPm10Grade=false;	// false로 초기화해주자
	}
	public void run(){
				...
				while(eventType!= XmlPullParser.END_DOCUMENT){	//문서의 끝이 아닐때
					...
					case XmlPullParser.TEXT:	//텍스트를 만났을때
						...
						if(bDate){				// 날짜
							...
							sSo2Value=xpp.getText();
							bSo2Value=false;
						}  if(bCoValue){				//일산화탄소 농도
							sCoValue=xpp.getText();
							bCoValue=false;
						}  if(bO3Value){				//오존 농도
							sO3Value=xpp.getText();
							bO3Value=false;
						}  if(bNo2Value){				//이산화질소 농도
							sNo2Value=xpp.getText();
							bNo2Value=false;
						}  if(bPm10Value){				//미세먼지 농도
							sPm10Value=xpp.getText();
							bPm10Value=false;
						}  if(bKhaiValue){				//통합대기 환경 수치
							sKhaiValue=xpp.getText();
							bKhaiValue=false;
						} if(bKhaiGrade){				//통합대기 환경 지수
							sKhaiGrade=xpp.getText();
							bKhaiGrade=false;
						}if(bSo2Grade){				//아황산가스 지수
							sSo2Grade=xpp.getText();
							bSo2Grade=false;
						}if(bCoGrade){				//일산화탄소 지수
							sCoGrade=xpp.getText();
							bCoGrade=false;
						}if(bO3Grade){				//오존지수
							sO3Grade=xpp.getText();
							bO3Grade=false;
						}if(bNo2Grade){				//이산화질소 지수
							sNo2Grade=xpp.getText();
							bNo2Grade=false;
						}if(bPm10Grade){				//미세먼지 지수
							sPm10Grade=xpp.getText();
							bPm10Grade=false;
						}if(bTotalCount){             // 총 합
		...
		handler.post(new Runnable() {	//기본 핸들러니깐 handler.post하면됨
			...
			public void run() {
				...
				if(tResponse){		//문서를 다 읽었다
					...
					WeatherFragment.FindDustThreadResponse(sTotalCount, sDate, sSo2Value, sCoValue, sO3Value, sNo2Value, sPm10Value, sKhaiValue, sKhaiGrade, sSo2Grade, sNo2Grade, sCoGrade, sO3Grade, sPm10Grade);
